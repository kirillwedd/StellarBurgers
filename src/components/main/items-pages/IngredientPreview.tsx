...
export function IngredientPreview({ingredientsPreview}: IngredientsPreview){
     ...
     const totalPrice = useMemo(() => {
        ...
        const maxIngredients = 5; // Максимальное количество ингредиентов для отображения
        ...
        const groupedIngredients = ingredientsBurger.reduce<Ingredient[]>((acc, ingredient) => {
            ...
            if (existing) {
                existing.count! += 1; // Увеличиваем счетчик на 1
            } else {
                acc.push({ ...ingredient, count: 1 }); // Добавляем новый ингредиент
            }
