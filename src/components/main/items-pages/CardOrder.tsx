...
export function CardOrder({id, ingredientsBurgers, createdAt, title, status, locationLink, onClick}: {id: number, ingredientsBurgers: Ingredient[], createdAt: string, title:string, status?: string, locationLink?: string, onClick?: ()=>void}){
    
    const time= new Date();
    const location =useLocation();
   
    return(
        <Link onClick={onClick} to={ locationLink==='профиль' ? `/profile/orders/${id}` : `/feed/${id}`} state={{ background: location}} className={styles.link}>
